#!/usr/bin/env bb
;; edn-datomize.bb — EDN → Datomic/Datascript tx-data 変換ツール（business-manager 用に adapt）。
...
;; gftdcojp/net-kotobase pilot と同型、Phase 3 の etzhayyim/com-etzhayyim-sanae 等と同型）。
;;
;; 使い方:
...
(defn merge-schema! [new-attrs]
  (let [existing (load-schema)
    ...
    (spit (schema-path) (str ";; schema.edn — Datomic/Datascript 互換スキーマ定義（自動生成 by edn-datomize.bb）\n"
                              ...
                              ";; edn-datomize.bb が変換した各種 .edn ファイル（actor/lex 等の\n"
                              ";; メタデータ）専用の付随スキーマ。\n\n"
                              (pr-str merged)
                              "\n"))
    merged))
